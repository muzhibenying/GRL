...
class GCNLayer(nn.Module):
    ...
    def forward(self, feat, node_view, adj):
        feat = self.fc(feat)
        node_view = self.fc(node_view)
        out = torch.bmm(adj, feat)
        node_view = torch.bmm(adj - torch.diag_embed(torch.diagonal(adj, dim1 = 1, dim2 = 2), dim1 = 1, dim2 = 2), node_view)
        if self.bias is not None:
            out += self.bias
            node_view += self.bias
        return self.act(out), node_view
...
class GCN(nn.Module):
    ...
    def forward(self, feat, adj, mask):
        h_1, node_view_1 = self.layers[0](feat, feat, adj)
        h_1g = torch.sum(h_1, 1)
        for idx in range(self.num_layers - 1):
            h_1, node_view_1 = self.layers[idx + 1](h_1, node_view_1, adj)
            h_1g = torch.cat((h_1g, torch.sum(h_1, 1)), -1)
        return h_1, h_1g, node_view_1
...
class Model(nn.Module):
    def __init__(self, n_in, n_h, num_layers):
        ...
        self.mlp3 = MLP(1 * n_h, n_h)
        self.mlp4 = MLP(1 * n_h, n_h)
        self.gnn1 = GCN(n_in, n_h, num_layers)
        self.gnn2 = GCN(n_in, n_h, num_layers)

    def forward(self, adj, diff, feat, mask):
        lv1, gv1, node_view_1 = self.gnn1(feat, adj, mask)
        lv2, gv2, node_view_2 = self.gnn2(feat, diff, mask)
        ...
        node_view_1 = self.mlp3(node_view_1)
        node_view_2 = self.mlp4(node_view_2)

        return lv1, gv1, lv2, gv2, node_view_1, node_view_2

    def embed(self, feat, adj, diff, mask):
        __, gv1, __, gv2, __, __ = self.forward(adj, diff, feat, mask)
...
# Borrowed from https://github.com/fanyun-sun/InfoGraph
def local_global_loss_(l_enc, g_enc, node_view, batch, measure, mask):
    '''
    Args:
        l: Local feature map.
        g: Global features.
    ...
    node_pos_mask = torch.zeros((num_nodes, num_nodes)).cuda()
    node_neg_mask = torch.zeros((num_nodes, num_nodes)).cuda()
    node_msk = torch.ones((num_nodes, num_nodes)).cuda()
    for nodeidx, graphidx in enumerate(batch):
        node_pos_mask[nodeidx][nodeidx] = 1
        node_neg_mask[nodeidx][nodeidx] = 1
    
    for idx, m in enumerate(mask):
        node_msk[idx * max_nodes + m: idx * max_nodes + max_nodes, idx] = 0.
    
    node_res = torch.mm(node_view, node_view.t()) * node_msk

    node_E_pos = get_positive_expectation(node_res * node_pos_mask, measure, average=False).sum()
    node_E_pos = node_E_pos / num_nodes
    node_E_neg = get_negative_expectation(node_res * node_neg_mask, measure, average=False).sum()
    node_E_neg = node_E_neg / (num_nodes * (num_nodes - 1))

    E_pos = get_positive_expectation(res * pos_mask, measure, average=False).sum()
    E_pos = E_pos / num_nodes
    E_neg = get_negative_expectation(res * neg_mask, measure, average=False).sum()
    E_neg = E_neg / (num_nodes * (num_graphs - 1))
    return E_neg - E_pos + node_E_neg - node_E_pos
...
def train(dataset, gpu, num_layer=4, epoch=40, batch=64):
    ...
    for epoch in range(nb_epochs):
        ...
        for idx in range(0, len(train_idx), batch_size):
            ...
            lv1, gv1, lv2, gv2, node_view_1, node_view_2 = model(adj[batch], diff[batch], feat[batch], mask)

            lv1 = lv1.view(batch.shape[0] * max_nodes, -1)
            lv2 = lv2.view(batch.shape[0] * max_nodes, -1)

            node_view_1 = node_view_1.view(batch.shape[0] * max_nodes, -1)
            node_view_2 = node_view_2.view(batch.shape[0] * max_nodes, -1)

            batch = torch.LongTensor(np.repeat(np.arange(batch.shape[0]), max_nodes)).cuda()

            loss1 = local_global_loss_(lv1, gv2, node_view_1, batch, 'JSD', mask)
            loss2 = local_global_loss_(lv2, gv1, node_view_2, batch, 'JSD', mask)
...
if __name__ == '__main__':
    ...
    with open("results.csv", "w") as f:
        f.write("Datasets,Layer,Batch,Epoch,Seed,Accuracy,std\n")
